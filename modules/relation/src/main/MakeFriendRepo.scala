...
private[relation] object MakeFriendRepo {
  ...
  def requester(userId: ID) = relaters(userId, Request)
  def requesting(userId: ID) = relating(userId, Request)
  ...
  def request(u1: ID, u2: ID): Funit = save(u1, u2, Request)
  def unrequest(u1: ID, u2: ID): Funit = remove(u1, u2)
